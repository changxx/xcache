...
package com.netease.xcache.interceptor;

import com.netease.xcache.XCache;
...
public abstract class CacheAspectSupport extends AbstractCacheInvoker
    ...
    private Object execute(CacheOperationInvoker invoker, CacheOperationContexts contexts) {
        ...
        if (result == null) {
            ...
            result = this.getResultWithPrevention(invoker, contexts.get(CacheableOperation.class));
        }
    ...
    /**
     * 防并发获取结果，保证同一个时间一个一个请求到数据库
     */
    private Cache.ValueWrapper getResultWithPrevention(CacheOperationInvoker invoker, Collection<CacheOperationContext> contexts) {
        Object result = ExpressionEvaluator.NO_RESULT;
        for (CacheOperationContext context : contexts) {
            if (isConditionPassing(context, result)) {
                Object key = generateKey(context, result);
                for (Cache cache : context.getCaches()) {
                    if (cache instanceof XCache) {
                        XCache xCache = (XCache) cache;
                        boolean success = xCache.setnx(key.toString() + "_lock", 1);
                        if (!success) {
                            // 获取失败，多次时间间隔从缓存取数据，到最大次数后返回空，时间间隔如果调整需要同步调整setnx的过期时间
                            for (int i = 0; i < 10; i++) {
                                try {
                                    Thread.sleep(100);
                                ...
                                Cache.ValueWrapper valueWrapper = findInCaches(context, key);
        ...
        // 执行程序返回结果
        return new SimpleValueWrapper(invokeOperation(invoker));
    }
